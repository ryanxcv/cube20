...
#ifdef SLICE
char cubepos::movefaces[FACES + 3] = {'U', 'F', 'R', 'D', 'B',
                                      'L', 'I', 'J', 'K'};
#endif
#ifdef AXIAL
char cubepos::movefaces[FACES + 9] = {'U', 'F', 'R', 'D', 'B', 'L', 'A', 'C',
                                      'E', 'G', 'H', 'I', 'J', 'K', 'M'};
#endif
...
void cubepos::init() {
  ...
  for (int s = 0; s < CANONSEQSTATES; s++) {
    ...
    for (int mv = 0; mv < NMOVES; mv++) {
      ...
      isplus = (mv % TWISTS == 0);
      if (s != 0 &&
          (prevface == f + 3 || (prevface == f && (prevplus != 1 || !isplus))))
#endif
#endif
      {
        canon_seq[s][mv] = INVALID;
...
#ifdef QUARTER
    canon_seq_mask_ext[s] =
        canon_seq_mask[s] | ((1LL << NMOVES_EXT) - (1LL << NMOVES));
...
#ifdef _WIN32
HANDLE pmutex;
void init_mutex() { pmutex = CreateMutex(NULL, FALSE, NULL); }
void get_global_lock() { WaitForSingleObject(pmutex, INFINITE); }
void release_global_lock() { ReleaseMutex(pmutex); }
HANDLE wthreads[MAX_THREADS];
void spawn_thread(int i, THREAD_RETURN_TYPE(THREAD_DECLARATOR *p)(void *),
                  void *o) {
  wthreads[i] = (HANDLE)_beginthreadex(NULL, 0, p, o, CREATE_SUSPENDED, NULL);
  ResumeThread(wthreads[i]);
}
void join_thread(int i) { WaitForSingleObject(wthreads[i], INFINITE); }
#else
pthread_mutex_t pmutex;
void init_mutex() { pthread_mutex_init(&pmutex, NULL); }
void get_global_lock() { pthread_mutex_lock(&pmutex); }
void release_global_lock() { pthread_mutex_unlock(&pmutex); }
